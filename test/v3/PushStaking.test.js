...
const { epnsContractFixture, tokenFixture } = require("../common/fixturesV3");
...
describe("PushCoreV3 Protocol", function () {
  ...
  beforeEach(async function () {
    ...
    ({ PROXYADMIN, EPNSCoreV1Proxy, EPNSCommV1Proxy, ROUTER, PushToken, PUSH_STAKING } =
      await loadFixture(epnsContractFixture));
  });

  describe("PushCoreV3: Stake and Claim Tests", () => {
    ...
    beforeEach(async function () {
      ...
      await PUSH_STAKING.connect(ADMINSIGNER).initializeStake();
    ...
    const stakePushTokens = async (signer, amount) => {
     await PushToken.connect(signer).approve(
        PUSH_STAKING.address,
        amount
     );
      await PUSH_STAKING.connect(signer).stake(amount);
    };

    const getLastStakedEpoch = async (user) => {
      const genesisEpoch = await EPNSCommV1Proxy.genesisEpoch();
    ...
    const claimRewardsInSingleBlock = async (signers) => {
      await ethers.provider.send("evm_setAutomine", [false]);
      await Promise.all(
        signers.map((signer) => PUSH_STAKING.connect(signer).harvestAll())
      );
      await network.provider.send("evm_mine");
      await ethers.provider.send("evm_setAutomine", [true]);
    };
    ...
    const getCurrentEpoch = async () => {
      const genesisBlock = await PUSH_STAKING.genesisEpoch()
      const currentBlock = await getCurrentBlock();

      const currentEpochNumber = await PUSH_STAKING.lastEpochRelative(
        genesisBlock,
        currentBlock.number
      );
    ...
    describe("🟢 lastEpochRelative Tests ", function () {
      it("Should revert on Block number overflow", async function () {
        ...
        const tx = PUSH_STAKING.lastEpochRelative(
          futureBlock.number,
          genesisBlock.number
        );
    ...
    //       await PUSH_STAKING.connect(ADMINSIGNER).addPoolFees(tokensBN(200));
    ...
    //       await PUSH_STAKING.connect(BOBSIGNER).harvestAll();
    //       await PUSH_STAKING.connect(ALICESIGNER).harvestAll();
    ...
    //         ALICE
    //       );
    //       const rewards_bob = await PUSH_STAKING.usersRewardsClaimed(BOB);
    //       const rewards_alice = await PUSH_STAKING.usersRewardsClaimed(
    //         ALICE
    //       );
