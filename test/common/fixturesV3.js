...
const epnsContractFixture = async ([adminSigner, others]) => {
  ...
  const EPNSCore = await ethers.getContractFactory("PushCoreV3");
  CORE_LOGIC = await EPNSCore.deploy();
  await CORE_LOGIC.deployed();

  const PushStaking = await ethers.getContractFactory("PushStaking");
  const PUSH_STAKING = await PushStaking.deploy(
    CORE_LOGIC.address,
    ADMIN,
    PushToken.address,
    ADMIN
  );
  await PUSH_STAKING.deployed();
// TODO: REMOVE LOG
console.log("PUSH_STAKING ADDRESS", PUSH_STAKING.address);
